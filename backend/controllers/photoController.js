...
module.exports = {
            ...
            .exec(function (err, photos) {
                if (err) {
                    return res.status(500).json({
                        message: 'Error when getting photo(list).',
                        error: err
                    });
                }
                var data = [];
    ...
    show: function (req, res) {
        ...
        PhotoModel.findOne({ _id: id }, function (err, photo) {
            if (err) {
                return res.status(500).json({
                    message: 'Error when getting photo(show).',
                    error: err
                });
            }
...
//            
    ...
    create: function (req, res) {
        ...
        photo.save(function (err, photo) {
            if (err) {
                return res.status(500).json({
                    message: 'Error when creating photo(create)',
                    error: err
                });
            }
    ...
    update: function (req, res) {
        ...
        PhotoModel.findOne({ _id: id }, function (err, photo) {
            if (err) {
                return res.status(500).json({
                    message: 'Error when getting photo(update)',
                    error: err
                });
            }
